...
let deleteLogIndexToConfirm = -1; 
let defaultFeeBeli = 0.11;
let defaultFeeJual = 0.11;
...
function switchTab(tabName) {
    ...
    Object.values(tabContents).forEach(content => content && content.classList.remove('active'));
...
function updateActiveSimDisplay() {
    ...
    displayContainer.innerHTML = `
        <div><span class="font-semibold text-gray-500">Kode:</span> <span class="text-blue-600 font-bold">${document.getElementById('stock-code').value.toUpperCase()}</span></div>
        <div><span class="font-semibold text-gray-500">Harga Awal:</span> <span class="font-medium">${formatCurrency(document.getElementById('initial-price').value)}</span></div>
        <div><span class="font-semibold text-gray-500">Lot Awal:</span> <span class="font-medium">${document.getElementById('initial-lot').value}</span></div>
        <div><span class="font-semibold text-gray-500">Avg. Down:</span> <span class="font-medium">${document.getElementById('avg-down-percent').value}%</span></div>
    `;
}
...
function calculateDashboard() {
    ...
    for (let i = 0; i <= avgLevels; i++) {
        ...
        row.innerHTML = `<td class="px-6 py-4 font-medium whitespace-nowrap">${i === 0 ? 'Entry Awal' : `AVG ${i}`}</td><td class="px-6 py-4 font-semibold text-yellow-700">${formatCurrency(entryPrice)}</td><td class="px-6 py-4">${lotsToBuy}</td><td class="px-6 py-4">${formatCurrency(totalBuy)}</td><td class="px-6 py-4 ${dividendYield > 5 ? 'text-green-600 font-bold' : 'text-gray-600'}">${dividendYield.toFixed(2)}%</td><td class="px-6 py-4 font-semibold text-blue-600">${formatCurrency(avgPrice)}</td><td class="px-6 py-4 text-green-600">${formatCurrency(tp1Price)}</td><td class="px-6 py-4 text-green-700 font-bold">${formatCurrency(tp2Price)}</td>`;
...
function renderLogTable(logsToRender = portfolioLog) {
    ...
    logsToRender.forEach((log, index) => {
        ...
        if (log.sellPrice) {
            ...
            statusHtml = `<td class="px-6 py-4"><span class="px-2 py-1 text-xs font-medium rounded-full bg-gray-200 text-gray-800">Closed</span></td>`;
            actionHtml = `<td class="px-6 py-4 flex space-x-2">
                            <button class="edit-log-btn p-1 text-blue-600 hover:text-blue-800" data-index="${portfolioLog.indexOf(log)}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg></button>
                            <button class="delete-log-btn p-1 text-red-600 hover:text-red-800" data-index="${portfolioLog.indexOf(log)}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" /></svg></button>
                          </td>`;
            sellDateHtml = `<td class="px-6 py-4">${log.sellDate}</td>`;
            feeJualDisplay = `<td class="px-6 py-4">${(log.feeJual || 0).toFixed(2)}%</td>`;
        } else {
            plHtml = `<td class="px-6 py-4 text-gray-400">-</td>`;
            statusHtml = `<td class="px-6 py-4"><span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">Open</span></td>`;
            actionHtml = `<td class="px-6 py-4 flex space-x-2">
                            <button class="sell-log-btn p-1 text-green-600 hover:text-green-800" data-index="${portfolioLog.indexOf(log)}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-8.707l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 001.414 1.414L9 9.414V13a1 1 0 102 0V9.414l1.293 1.293a1 1 0 001.414-1.414z" clip-rule="evenodd" /></svg></button>
                            <button class="edit-log-btn p-1 text-blue-600 hover:text-blue-800" data-index="${portfolioLog.indexOf(log)}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg></button>
                            <button class="delete-log-btn p-1 text-red-600 hover:text-red-800" data-index="${portfolioLog.indexOf(log)}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" /></svg></button>
                          </td>`;
            sellDateHtml = `<td class="px-6 py-4 text-gray-400">-</td>`;
            feeJualDisplay = `<td class="px-6 py-4 text-gray-400">-</td>`;
        }
        
        row.innerHTML = `
            <td class="px-6 py-4">${log.date}</td>
            <td class="px-6 py-4 font-medium text-blue-700">${log.code.toUpperCase()}</td>
            <td class="px-6 py-4">${formatCurrency(log.price)}</td>
            <td class="px-6 py-4">${log.lot}</td>
            <td class="px-6 py-4">${(log.feeBeli || 0).toFixed(2)}%</td>
            ${feeJualDisplay}
            <td class="px-6 py-4 text-gray-500">${log.reason || '-'}</td>
            ${statusHtml}
            ${sellDateHtml}
            ${plHtml}
            ${actionHtml}
...
function renderFinancialSummaries() {
    ...
    if (openPositions.length === 0) {
        ...
        grandTotalContainer.innerHTML = `<div class="p-4 bg-gray-100 rounded-lg text-center"><h4 class="text-gray-500 font-medium">Total Investasi (Open)</h4><p class="text-2xl font-bold text-yellow-600 mt-1">${formatCurrency(0)}</p></div>`;
    } else {
        ...
        Object.entries(summary).forEach(([code, data]) => {
            ...
            summaryCard.className = 'p-4 bg-white/70 rounded-lg border border-gray-200';
            summaryCard.innerHTML = `<h4 class="font-bold text-lg text-blue-700">${code}</h4><div class="flex justify-between text-sm mt-2"><span class="text-gray-500">Total Lot:</span><span class="font-semibold">${data.totalLots}</span></div><div class="flex justify-between text-sm mt-1"><span class="text-gray-500">Harga Rata-rata:</span><span class="font-semibold">${formatCurrency(avgPriceWithFee)}</span></div><div class="flex justify-between text-sm mt-1"><span class="text-gray-500">Total Investasi:</span><span class="font-semibold">${formatCurrency(data.totalCost)}</span></div><div class="flex justify-between items-center text-sm mt-2"><label for="current-price-${code}" class="text-gray-500">Harga Saat Ini:</label><input type="number" id="current-price-${code}" data-code="${code}" class="current-price-input w-24 bg-white border border-gray-300 rounded p-1 text-right" placeholder="0" value="${priceValue}"></div><div class="flex justify-between text-sm mt-1"><span class="text-gray-500">Floating P/L:</span><span id="floating-pl-${code}" class="font-semibold">-</span></div>`;
            summaryContainer.appendChild(summaryCard);
        });
        const grandTotalCard = document.createElement('div');
        grandTotalCard.className = 'p-4 bg-gray-100 rounded-lg text-center';
        grandTotalCard.innerHTML = `<h4 class="text-gray-500 font-medium">Total Investasi (Open)</h4><p class="text-2xl font-bold text-yellow-600 mt-1">${formatCurrency(grandTotalCost)}</p></div>`;
    ...
    realizedPlCard.className = 'p-4 bg-gray-100 rounded-lg text-center';
    const plColor = totalRealizedPL >= 0 ? 'text-green-600' : 'text-red-600';
    realizedPlCard.innerHTML = `<h4 class="text-gray-500 font-medium">Total Realisasi P/L</h4><p class="text-2xl font-bold ${plColor} mt-1">${formatCurrency(totalRealizedPL, true)}</p>`;
...
function calculateAndRenderFloatingPL() {
    ...
    Object.entries(summary).forEach(([code, data]) => {
        ...
        if (currentPrice > 0) {
            ...
            const plColor = floatingPL >= 0 ? 'text-green-600' : 'text-red-600';
    ...
    const plColor = totalFloatingPL >= 0 ? 'text-green-600' : 'text-red-600';
    floatingPlContainer.innerHTML = `<div class="p-4 bg-gray-100 rounded-lg text-center"><h4 class="text-gray-500 font-medium">Total Floating P/L</h4><p class="text-2xl font-bold ${plColor} mt-1">${formatCurrency(totalFloatingPL, true)}</p></div>`;
    renderPerformanceTab();
}
...
function renderPerformanceTab() {
    ...
    periods.forEach(period => {
        ...
        const performanceCell = isAllTime && isFinite(portfolioPerformance) ? `<td class="px-6 py-4 font-semibold ${portfolioPerformance >= 0 ? 'text-green-600' : 'text-red-600'}">${portfolioPerformance.toFixed(2)}%</td>` : `<td class="px-6 py-4 text-gray-400">N/A</td>`;
        row.innerHTML = `<td class="px-6 py-4 font-medium">${period}</td>${performanceCell}<td class="px-6 py-4"><input type="number" class="performance-ihsg-input w-24 bg-white border border-gray-300 rounded p-1 text-right" data-period="${period}" placeholder="0.00"></td><td class="px-6 py-4 font-semibold" id="performance-diff-${period.replace(/\s+/g, '')}">-</td>`;
        tableBody.appendChild(row);
    });
...
function calculatePerformanceDifference() {
     ...
     document.querySelectorAll('.performance-ihsg-input').forEach(input => {
        ...
        if (!isNaN(ihsgPerf) && period === 'All Time' && isFinite(allTimeReturn)) {
            ...
            diffCell.className = `px-6 py-4 font-semibold ${difference >= 0 ? 'text-green-600' : 'text-red-600'}`;
...
function handleEditLog(index) {
    ...
    if (logToEdit) {
        ...
        logFeeBeliInput.value = logToEdit.feeBeli || defaultFeeBeli;
        ...
        if (logToEdit.sellPrice !== null) {
            ...
            logFeeJualInput.value = logToEdit.feeJual || defaultFeeJual;
        } else {
            sellFieldsContainer.classList.add('hidden');
            logSellPriceInput.value = '';
            logSellDateInput.value = '';
            logFeeJualInput.value = defaultFeeJual;
        }
...
function renderSavedSimulationsTable() {
    ...
    savedSimulations.forEach(sim => {
        const row = document.createElement('tr');
        row.className = 'border-b border-gray-200 hover:bg-gray-50';
        row.innerHTML = `<td class="px-6 py-4 font-medium text-blue-700">${sim.stockCode.toUpperCase()}</td><td class="px-6 py-4">${formatCurrency(sim.initialPrice)}</td><td class="px-6 py-4">${sim.initialLot}</td><td class="px-6 py-4">${sim.avgDownPercent}%</td><td class="px-6 py-4">${sim.avgLevels}</td><td class="px-6 py-4 space-x-4"><button class="load-sim-btn text-green-600 hover:text-green-800 font-semibold" data-id="${sim.id}">Muat</button><button class="delete-sim-btn text-red-600 hover:text-red-800 font-semibold" data-id="${sim.id}">Hapus</button></td>`;
        tableBody.appendChild(row);
    });
}
        ...
        filteredLogs.sort((a, b) => {
            ...
            switch (sortState.column) {
                ...
                default: // Numeric columns
                    aValue = a[sortState.column] || 0;
                    bValue = b[sortState.column] || 0;
                    break;
            }
...
logForm.addEventListener('submit', handleAddOrEditLog);
...
document.getElementById('log-table-body').addEventListener('click', (event) => {
    ...
    if (button.classList.contains('delete-log-btn')) {
        ...
        document.getElementById('sell-fee-jual').value = portfolioLog[logIndex].feeJual || defaultFeeJual;
        openModal(sellModal);
    } else if (button.classList.contains('edit-log-btn')) {
...
filterStockCodeInput.addEventListener('input', applyLogFiltersAndSort);
filterReasonInput.addEventListener('input', applyLogFiltersAndSort);
