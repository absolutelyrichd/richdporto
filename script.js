...
const tabButtons = { 
    ...
    dev: document.getElementById('tab-btn-dev') // NEW DEV TAB BUTTON
};
const tabContents = { 
    ...
    dev: document.getElementById('tab-content-dev') // NEW DEV TAB CONTENT
};
...
function switchTab(name) {
    ...
    if(name === 'dev') renderDevTab(); // CALL DEV RENDERER
}

// --- LOGIC: DEV TAB (NEW) ---
function renderDevTab() {
    // 1. System Info
    const uidEl = document.getElementById('dev-uid');
    const syncStatusEl = document.getElementById('dev-sync-status');
    uidEl.textContent = currentUser ? currentUser.uid : 'Not Logged In';
    
    // 2. Memory Calculation (Rough Estimate)
    const dataString = JSON.stringify({ portfolioLog, savedSimulations, currentMarketPrices });
    const bytes = new Blob([dataString]).size;
    const kb = (bytes / 1024).toFixed(2);
    document.getElementById('dev-storage-size').textContent = `${kb} KB`;

    // 3. State Inspector
    document.getElementById('dev-json-log').value = JSON.stringify(portfolioLog, null, 2);
    document.getElementById('dev-json-sim').value = JSON.stringify(savedSimulations, null, 2);
}

function devGenerateDummyData() {
    if(!confirm("WARNING: Ini akan menambahkan data dummy ke jurnal Anda. Lanjutkan?")) return;
    
    const dummyStocks = ['BBCA', 'BBRI', 'TLKM', 'ASII', 'BMRI'];
    const dummyData = [];
    for(let i=0; i<5; i++) {
        const code = dummyStocks[Math.floor(Math.random() * dummyStocks.length)];
        const price = 5000 + Math.floor(Math.random() * 5000);
        const lot = 10 + Math.floor(Math.random() * 90);
        dummyData.push({
            id: Date.now() + i,
            code: code,
            ...
            reason: "Generated by Dev Tools",
            sellPrice: null, sellDate: null, feeJual: null
        });
    }
    portfolioLog = [...portfolioLog, ...dummyData];
    refreshData();
    showNotification("5 Data Dummy berhasil ditambahkan!");
    renderDevTab();
}

function devHardReset() {
    if(!confirm("DANGER ZONE: Apakah Anda yakin ingin MENGHAPUS SEMUA DATA? Tindakan ini tidak bisa dibatalkan.")) return;
    if(!confirm("Konfirmasi kedua: Benar-benar yakin hapus semua?")) return;
    
    portfolioLog = [];
    savedSimulations = [];
    currentMarketPrices = {};
    refreshData();
    triggerAutoSave(); // Sync empty state to cloud
    showNotification("System Hard Reset Successful.");
    renderDevTab();
}
...
window.addEventListener('load', () => {
    ...
    // NEW: Dev Tools Event Listeners
    document.getElementById('dev-generate-dummy').addEventListener('click', devGenerateDummyData);
    document.getElementById('dev-hard-reset').addEventListener('click', devHardReset);
